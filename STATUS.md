...
Session 53: **eBay discovery filter** — Dropped **past-only** `itemEndDate` on Browse search. **Baseball** uses **`category_ids=261328`** as a **query param** (not `categoryId` inside `filter`, which was yielding **total=0**). **`EbayScraper`** sends **`X-EBAY-C-MARKETPLACE-ID`** / **`ENDUSERCTX`** for **US**. Discovery **fallback** retry without `buyingOptions` if first call returns 0. **`find_opportunities`**: **exit 1** if **0 players** (error text points at **`DISCOVER_SUMMARY`** + **`error_log`** categories). **`discover_players`**: **`DISCOVER_SUMMARY`** JSON line per run; **`error_log`** on all-seeds-zero / Browse HTTP–API / exceptions / degraded partial success; **Browse 429** with **`Retry-After`** capped (default **25s**/wait, override **`EBAY_DISCOVER_429_MAX_SLEEP`**) + **0.5s** pacing. **`scripts/dev/ebay_browse_ping.py`**: one Browse call to see **200 vs 429**; portal for daily totals (**`PIPELINE-OPS.md`**). **`itemEndDate`** filters use UTC **`.000Z`** in **`ebay_scraper`**.
